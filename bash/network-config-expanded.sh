...
declare -a j=($interfaces)
len=${#j[*]}
i=0
#Making a loop to run each job in the array
while [ $i -lt $len ]
do
    if [[ ${j[$i]} = lo* ]] ; then continue ;
	#Implementing the code conditions to avoid loopback
	fi
	#Basic lines to set the function of the command to the variable
	 ipv4_address=$(ip a s ${j[$i]} | awk -F '[/ ]+' '/inet /{print $3}')
	 ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
	 network_address=$(ip route list dev ${j[$i]} scope link|cut -d ' ' -f 1)
