...
#Moving the command output to separate variables and clean the script
myhostname=$(hostname) 
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #Looking for interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #Looking for Lan address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #Using the getent hosts and variable $lan_address to find Lan Hostname
#Generating external IP address using curl
external_ip=$(curl -s icanhazip.com)
external_name=$(getent hosts $external_ip| awk '{print $2}') #Generating external ip by scanning the hosts file using $external_ip
router_address=$(ip r s default| cut -d ' ' -f 3)
#Getting a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}')
#Getting the router hostname by getent host and $router_address
